...
- `index.html` gained the Sync button + a status line.
- **Bug #1 found and fixed during smoke-testing**: `upload_track` (step 5) was decoding straight from an in-memory `BytesIO`, which works for WAV (libsndfile handles it natively) but silently breaks for the compressed formats real phones actually record — webm/opus (Android) or mp4/aac (iOS) — because those need soundfile's audioread/ffmpeg fallback, and ffmpeg is invoked as a subprocess against a real file path, not a byte stream. Confirmed the failure by generating a synthetic `.webm` clip (`ffmpeg -c:a libopus`) and POSTing it exactly as `app.js`'s `syncOneCapture` would — got `400: Format not recognised`. Fixed by writing the upload to a `tempfile.NamedTemporaryFile` before decoding (`server.py`). README updated to note `ffmpeg` as a hard requirement on the machine running the server.
- **Bug #2 found live, on the real phone**: the actual end-to-end test (see below) showed two full upload+annotate cycles in the server log resolving to the exact same auto-generated track id (`field_20260718_215434`), which is only possible if `generate_track_id()`'s 1-second resolution collided across two near-simultaneous requests — most likely the opportunistic on-load sync and a manual Sync click both starting before the first had a chance to disable the button (`syncBtn.disabled = true` happened after an `await`, leaving a race window). Fixed both ends: `naming.py`'s `generate_track_id()` now includes microseconds (`field_YYYYMMDD_HHMMSS_ffffff`), and `app.js`'s `syncPendingCaptures()` now sets a module-level `syncInFlight` guard synchronously, before any `await`, so overlapping calls can't both proceed. `sw.js` cache bumped to `v4` to ship the fix.
- Full pytest suite: 89 passed (added a collision-focused test — 100 calls to `generate_track_id()` are all unique).
- **Verified end-to-end on the real iPhone**: WiFi → airplane mode → record + tap + Save → confirmed "queued locally" with zero network activity → back on WiFi → Sync → server log shows the real `POST /datasets/Swing/tracks` + `.../annotations` requests landing (audioread/ffmpeg fallback firing exactly as expected for a real webm/opus recording) → confirmed on disk: `data/swing/tracks/field_*.wav` + matching `.beats` files exist, with plausible beat timestamps, and `/datasets` reflects the updated track/annotation counts. This closes the loop the whole 10-step plan was building toward. (Android device and the desktop-annotator visual check are still worth doing whenever convenient, but the core mechanism is now confirmed working.)
