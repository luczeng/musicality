...
def build_dataloaders(cfg: DictConfig) -> tuple[DataLoader, DataLoader, int, int]:
    ...
    # Same cache key scheme as train_beat_phase.py's beat_phase-<name>[-binary]:
    # BeatDataset's filtering (and therefore its length) only depends on
    # name/binary_only, not on which head(s) a given trainer uses, so reusing
    # the identical key here means beat-only and beat-phase runs share the
    # exact same held-out split for the same dataset — useful for comparing
    # the two directly.
    ...
    if subsample is not None:
        ...
        print(
            f"[train_beat_only] Subsampled train set: {n}/{n_before} ({subsample:.0%})"
        )
