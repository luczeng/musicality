...
def _target(beat=0.0, one=0.0, last=0.0, mask=1.0) -> torch.Tensor:
    t = torch.zeros(B, 4, T)
    t[:, 0] = beat
    t[:, 1] = one
    t[:, 2] = last
...
class TestBeatPhaseLoss:
    ...
    def test_loss_is_positive(self):
        loss = beat_phase_loss(_logits(), _target(beat=1.0, one=1.0, last=1.0))
        assert loss.item() > 0.0

    def test_perfect_prediction_lower_than_wrong(self):
        target = _target(beat=1.0, one=1.0, last=1.0)
    ...
    def test_gradients_flow(self):
        logits = _logits()
        loss = beat_phase_loss(logits, _target(beat=1.0, one=1.0, last=1.0))
        loss.backward()
        assert logits.grad is not None
        assert torch.isfinite(logits.grad).all()

    def test_mask_zero_ignores_one_last_errors(self):
        """When mask=0, one/last predictions must not affect the loss at all."""
        logits = _logits(0.0)

        target_masked_wrong = _target(beat=1.0, one=1.0, last=1.0, mask=0.0)
        target_masked_matching_beat_only = _target(
            beat=1.0, one=0.0, last=0.0, mask=0.0
        )
