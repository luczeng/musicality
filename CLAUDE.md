...
- `leaderboard.py` — Compares *several* runs, where `eval_beat.py` scores one. Walks checkpoint folders, re-evaluates every run it finds through `BeatEvaluator` (so the numbers match `eval_beat.py`) on the split `configs/eval_beat.yaml` names, sweeping each checkpoint's own postprocessing on the **train** split first so the reported val numbers stay held out — unlike `eval_beat.py --sweep`, which tunes and reports on the same tracks. Results merge into one **running** board that lives on W&B: a single `leaderboard.json`, fetched from the `musicality-leaderboard` project before reading and published back after writing, so it outlives the instance that produced it. Each publish is one run holding the board twice — a `wandb.Table` to sort and filter in the browser, and the JSON itself as a versioned artifact whose `:latest` is what the next invocation fetches. Locally it is a working copy at `leaderboard/leaderboard.json` (gitignored); `--board` puts it elsewhere, which makes it a throwaway comparison, never fetched from or published to W&B; `--no-fetch`/`--no-publish` skip either half, and `--publish-only` publishes the board already on disk, scoring nothing. Each row carries the W&B run that trained it (`wandb_name`/`wandb_url`/`train_commit`, read from the `training_report.json` beside its checkpoints), so a leading number links back to the experiment behind it. Only the runs named on the command line are evaluated, the rest are carried over, and rows measured under different settings are refused rather than merged.
- `eval_beat.py` — The one evaluation tool for beat-only/beat-phase checkpoints (task auto-detected), on full-length tracks. Default is the canonical metric report; `--per-genre` (automatic on a merged split) breaks it down per corpus, `--profile` prints the phase-offset profile, `--decoders` scores every bar-position decoder against one cached model pass and calls model-vs-decoder, `--sweep` grid-searches the postprocessing knobs that `configs/eval_beat.yaml` holds, `--output` writes per-track rows to CSV. Every mode runs the model once per track and re-uses the cached probabilities. Runs on the GPU by default (`device: auto` in `configs/eval_beat.yaml`, resolved by `musicality.inference.resolve_device`), decodes audio a few tracks ahead on threads (it costs more than the forward pass), and spreads a sweep's grid points across processes when they cost more than starting them — see `docs/source/workflows.rst` ("Where an evaluation spends its time").
